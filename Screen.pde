...
class Screen {
    ...
    //podaci za sat
    int cx, cy, prvi_prolazak=0, cy_new;
    float secondsRadius, clockDiameter;
    String comboWord;
    int comboWordFrameCount, comboWordSize;
    float centerRelativeX=10;
    float centerRelativeY=27;
    ...
    void draw()
    {
        ...
        //Zatim crtamo rubove ekrana - zaobljene pravokutnike 
        ...
        if(level==0)
          ...
          drawClock(level,levelTimer); //drawClock(1,levelTimer); 
    ...
    void drawComboWords()
    {
        if(comboWordFrameCount > 0)
        {
            ...
            textWithOutline(comboWord, width/2, height/2+200, color(variable, 255, 255), 50);
            comboWordFrameCount--;
            drawStars();
        }
    }
    ...
    void drawClock(int lvl, int timer)
    {
      float s = map(timer/30, 0, 60, 0, TWO_PI)-HALF_PI;
      float angle = map(timer/30, 0, 60, 0, TWO_PI);
      //s_1 je kut od 0 do 2pi;
      image(clockEmpty, cx, cy);

      setClockHandle(clockHandle, angle);
    ...
    void hurry()
    {
      ...
      textWithOutline("Hurry up!", width/2+r, height/2+r, color(variable, 255, 255), 60);
      hurry_up.play();

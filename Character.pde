...
class Character {
        ...
        else
        {
            if (onGround)
            {
                if (abs(vx) < 1) // Ako je na zemlji i ne krece se
                {
                    ...
                    Platform currPlatform = screen.getPlatforms().get(currentPlatformIndex);
                    int isPlayerOnLedge = currPlatform.isOnLedge(this);
                    if (abs(isPlayerOnLedge) == 1 && !currPlatform.getIsWide())
                    {
    ...
    void drawCombo()
    {
        ...
        if(comboCount > 0)
        {
            textWithOutline(str(comboCount) + "\n FLOORS!", 75, 270, color(variable, 255, 255), 24);
        }
       ...
       /* fill(#800040);
        ...
        image(originalTimer, 75, 150);
        fill(#fd4102);
        rect(61, 83 + 180*0.75 - comboTimer*0.75, 20, comboTimer*0.75, 8); //skaliramo comboTimer td lijepo stane u originalTimer

        // Trenutni najveci combo
        textWithOutline("Best\ncombo:\n" + str(round(highestCombo)), 75, 550, color(variable, 255, 255), 25);

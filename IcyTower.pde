//state=0 će biti početni ekran
//state=1 je igra
//state=2 je game over screen
//state=3 je pause screen
//state=4 su instrukcije
...
int state = 0, variable=0, currentLetter=0, pickedOption=0, currentMenuOptionsCount;
int cursorOriginalHeight;
PFont icyFont, icyFontFill;
PImage bg, bg2, cursorHarold, instructionsImage, clockEmpty, clockHandle, originalTimer;
Screen mainScreen;
Character player;
...
void setup()
{
    ...
    cursorOriginalHeight = cursorHarold.height;
...
void startScreen()
{
    ...
    if(pickedCharacter=="wild_wendy")pickedCharacter="Wendy";
    ...
    textWithOutline("Play", 70, 1*height/12,                                 color(variable, 255, 255), 40);
    textWithOutline("Character: <" + pickedCharacter + ">", 70, 2*height/12, color(variable, 255, 255), 40);
    textWithOutline("Instructions", 70, 3*height/12,                         color(variable, 255, 255), 40);
    textWithOutline("Exit ", 70, 4*height/12,                                color(variable, 255, 255), 40);
    
    int cursorWidth = 25;
    int cursorHeight = (pickedOption+1)*height/12 - 2*cursorOriginalHeight/3 - 10;
    image(cursorHarold, cursorWidth, cursorHeight);

    textAlign(CENTER);
    rotate(PI/6);
    textWithOutline("ICY", 4*width/5, -height/2+40,       color(variable, 255, 255), 130);
    textWithOutline("TOWER", 4*width/5, -height/2+200, color(variable, 255, 255), 130);
    rotate(-PI/6);
    ...
    if ( keyPressed && key == ENTER && enterReleased  )
    {
        ...
        switch (pickedOption) {
            ...
            case 0:
                if (pickedCharacter == "Dave") pickedCharacter = "dave";
                else if (pickedCharacter == "Harold") pickedCharacter = "harold";
                else if (pickedCharacter == "Wendy") pickedCharacter = "wild_wendy";
            ...
            break;
            // Ciklicko mijenjanje likova - treba promijeniti i kod nize gdje su bindovi za hotkey RIGHT i LEFT
            case 1:
                if (pickedCharacter == "Dave") pickedCharacter = "Wendy";
                else if (pickedCharacter == "Harold") pickedCharacter = "Dave";
                else if (pickedCharacter == "Wendy") pickedCharacter = "Harold";
...
void endScreen()
{
    ...
    textAlign(LEFT);
    textWithOutline("Play again", width/3, height/3+50,     color(variable, 255, 255), 50);
    textWithOutline("Main menu", width/3, height/3+50 + 50, color(variable, 255, 255), 50);
    textWithOutline("Exit", width/3, height/3+50 + 100,     color(variable, 255, 255), 50);

    image(cursorHarold, width/3 - 30, height/3+50 + (pickedOption)*50 - cursorHarold.height/2 + 7);
    ...
    if(player.isThereANewRecord() && !usernameEntered)
    {
        ...
        // Dodaj rekord
        if (state == 2 && !usernameEntered && keyPressed && key == ENTER && enterReleased)
        {
            enterReleased=false;
            playSelectSound();
...
void keyPressed() {
    if (key==CODED)
    {
        ...
        if(state == 1 || state == 3) return; // U stanjima 1 i 3 nema menija
        ...
        // Mijenja odabir lika
        if(state == 0 && (keyCode==LEFT || keyCode==RIGHT) && pickedOption == 1)
        {
            // ciklicko mijenjanje likova
            if (pickedCharacter == "Dave") pickedCharacter = "Wendy";
            else if (pickedCharacter == "Harold") pickedCharacter = "Dave";
            else if (pickedCharacter == "Wendy") pickedCharacter = "Harold";
            playSelectSound();
        }
        else if((state == 0 || state == 2) && (keyCode==UP || keyCode==DOWN || (state == 0 && (keyCode==LEFT || keyCode==RIGHT) && pickedOption == 1)) )
        {
            menu_option.play();
    ...
    } else if ((key == 'p' || key == 'P') && (state == 1 || state == 3)) // Pause screen on/off
    {
        state = (state == 1) ? 3 : 1;
    } else if (state == 2 && !usernameEntered && key!=ENTER) // Ako je rekord onda upis slova za username
    {
